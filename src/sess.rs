            ...
            .map(|file| match *file {
                config::SourceFile::File(ref path) => {
                    let ty = match path.extension().and_then(std::ffi::OsStr::to_str) {
                        // `.vh` / `.svh` headers are de-facto SystemVerilog macro files;
...
impl<'io, 'sess: 'io, 'ctx: 'sess> SessionIo<'sess, 'ctx> {
    ...
    fn git_database_action(
        ...
        std::fs::create_dir_all(db_dir)
            .into_diagnostic()
            .wrap_err_with(|| format!("Failed to create git database directory {:?}.", db_dir))?;
        let git = Git::new(
            db_dir,
                ...
                if self.sess.config.git_lfs {
                    ...
                    match uses_lfs {
                        Ok(true) => {
                            local_git
                                .clone()
                                .spawn_with(move |c| c.arg("config").arg("lfs.url").arg(url), None)
                                .await?;
                            local_git
                                .clone()
                                .spawn_with(move |c| c.arg("lfs").arg("pull"), None)
                                .await?;
                        }
                        Ok(false) => {}
                        Err(cause) => {
